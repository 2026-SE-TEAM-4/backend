...
def _to_hourly_series(rows: list) -> pd.Series | None:
    ...
    # interpolate() 는 양끝 사이의 결측만 채운다. 선두에 빈 버킷이 있으면 NaN 이
    # 남아 적합이 깨지므로, 가장 가까운 뒤쪽 값으로 backfill 해 결측을 없앤다.
    series = series.bfill()
    return series if not series.empty else None


def _to_hourly_count_series(rows: list, now: datetime) -> pd.Series | None:
    ...
    if not rows:
        return None
    index = pd.DatetimeIndex([row[0] for row in rows])
    counts = pd.Series(1.0, index=index).resample("1h").sum()
    # 마지막 예약 이후 최근 시간대처럼 예약이 0인 구간은 리샘플 결과에서 아예 빠진다.
    # 그대로 두면 "수요가 끊긴 사실"이 사라져 추세가 왜곡되므로, [now-30d, now] 고정
    # 격자에 맞춰 reindex 하고 빈 시간은 0건으로 채워 0수요를 명시적으로 표현한다.
    grid = pd.date_range(now - _HISTORY_WINDOW, now, freq="1h")
